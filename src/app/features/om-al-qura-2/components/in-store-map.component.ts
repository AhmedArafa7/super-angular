...
@Component({
  selector: 'app-om-al-qura-2-in-store-map',
  standalone: true,
  imports: [CommonModule, FormsModule, LucideDynamicIcon, ImageFallbackDirective],
  template: `
    <div class="space-y-8 font-sans" dir="rtl">
      
      <!-- Interactive Navigation Map Header for Factory -->
      <div class="bg-gradient-to-br from-slate-900 via-zinc-900 to-amber-950 rounded-3xl p-8 text-white shadow-2xl relative overflow-hidden border border-amber-500/30">
        <div class="relative z-10 max-w-3xl space-y-4">
          <div class="inline-flex items-center gap-2 px-3 py-1 bg-amber-500/20 backdrop-blur-md rounded-full text-xs font-bold text-amber-300 border border-amber-400/30">
            <svg lucideIcon="map-pin" class="w-3.5 h-3.5 text-amber-400 animate-pulse"></svg>
            <span>دليل خريطة مصنع محمود عرفه للمعادن والمخازن الهندسية</span>
          </div>
          <h2 class="text-3xl font-black">ابحث عن أي قطاع معدني أو خام وتوجّه لمكانه بالمصنع أو المخزن</h2>
          <p class="text-xs text-amber-200/80 leading-relaxed">
            اختر المنتجات المعدنية التي تبحث عنها، وسيقوم النظام بتحديد موقعها الدقيق (الممر، الرف، حالة التوفر، والمخزن الهندسي للمصنع).
          </p>
      ...
      <div class="bg-white dark:bg-slate-900 rounded-3xl p-6 border border-slate-200 dark:border-slate-800 shadow-md space-y-4">
        ...
        <div class="relative bg-slate-950 rounded-3xl overflow-hidden border-2 border-slate-800 p-2 min-h-[300px] flex items-center justify-center">
          ...
          <ng-template #noSketchPlaceholder>
            <div class="p-8 text-center space-y-3 text-slate-200">
              <div class="w-14 h-14 rounded-2xl bg-amber-500/20 border border-amber-500/40 flex items-center justify-center mx-auto text-amber-400">
                <svg lucideIcon="grid" class="w-7 h-7"></svg>
              </div>
              <h4 class="font-black text-amber-300 text-base">لم يتم رسم كروكي بالخطوط المستقيمة حتى الآن</h4>
              <p class="text-xs text-amber-100 font-bold max-w-md mx-auto leading-relaxed">
                اضغط على زر "فتح لوحة رسم الخطوط بالنقطتين" لوضع نقطة البداية (A) ونقطة النهاية (B) ليرسم النظام خطاً مستقيماً دقيقاً بينهما!
              </p>
              <button (click)="openLineCanvasDrawer()" class="px-5 py-2.5 bg-amber-500 hover:bg-amber-400 text-slate-950 font-black rounded-2xl text-xs shadow-lg inline-flex items-center gap-2 cursor-pointer transition-all">
                <svg lucideIcon="pen-tool" class="w-4 h-4"></svg>
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-3 text-xs pt-2">
          <div class="p-3.5 bg-emerald-500/10 dark:bg-emerald-950/40 rounded-2xl border-2 border-emerald-500/50 space-y-1">
            <span class="text-emerald-800 dark:text-emerald-300 font-black text-xs block">🚪 المدخل الرئيسي:</span>
            <p class="font-black text-slate-900 dark:text-white text-sm leading-snug">{{ service.storeLayout().storeEntranceLabel }}</p>
          </div>
          <div class="p-3.5 bg-amber-500/10 dark:bg-amber-950/40 rounded-2xl border-2 border-amber-500/50 space-y-1">
            <span class="text-amber-800 dark:text-amber-300 font-black text-xs block">💵 منطقة الكاشير والاستقبال:</span>
            <p class="font-black text-slate-900 dark:text-white text-sm leading-snug">{{ service.storeLayout().checkoutAreaLabel }}</p>
          </div>
          <div class="p-3.5 bg-indigo-500/10 dark:bg-indigo-950/40 rounded-2xl border-2 border-indigo-500/50 space-y-1">
            <span class="text-indigo-800 dark:text-indigo-300 font-black text-xs block">📦 المخزن الداخلي:</span>
            <p class="font-black text-slate-900 dark:text-white text-sm leading-snug">{{ service.storeLayout().warehouseAreaLabel }}</p>
          </div>
        </div>
      </div>
      ...
      <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
              ...
              <div class="flex-1 min-w-0">
                ...
                <div class="flex items-center gap-2 mt-2">
                  <span class="px-2 py-0.5 rounded-full text-[10px] font-bold"
                        [ngClass]="p.stockQuantity > 0 ? (p.isInWarehouse ? 'bg-amber-100 text-amber-800' : 'bg-amber-500 text-slate-950') : 'bg-rose-100 text-rose-800'">
                    {{ p.stockQuantity > 0 ? (p.isInWarehouse ? 'متوفر بالمخزن الهندسي' : 'متوفر بالمصنع والمعرض') : 'نفد من المخزون' }}
                  </span>
                </div>
              </div>
            </div>
        ...
        <div class="lg:col-span-2 space-y-6">
          <div *ngIf="effectiveSelectedProduct()" class="bg-white dark:bg-slate-900 rounded-3xl p-6 border border-slate-200 dark:border-slate-800 shadow-sm space-y-6">
            ...
            <div class="flex flex-wrap justify-between items-start gap-4 p-5 bg-gradient-to-r from-slate-900 via-zinc-900 to-amber-950 text-white rounded-2xl border border-amber-500/30 shadow-md">
              <div>
                <span class="text-xs font-bold text-amber-300">موقع القطاع أو الخام المعدني المحدد حالياً:</span>
                <h3 class="text-2xl font-black text-white mt-1">{{ effectiveSelectedProduct()?.name }}</h3>
              ...
              <div class="text-left">
                <span class="inline-block px-4 py-2 rounded-2xl text-xs font-black"
                      [ngClass]="(effectiveSelectedProduct()?.stockQuantity || 0) > 0 ? 'bg-amber-500 text-slate-950' : 'bg-rose-600 text-white'">
                  {{ (effectiveSelectedProduct()?.stockQuantity || 0) > 0 ? 'متوفر بالمصنع الآن' : 'غير متوفر حالياً' }}
                </span>
                <p *ngIf="(effectiveSelectedProduct()?.stockQuantity || 0) <= 0" class="text-xs text-rose-400 font-bold mt-2">
            ...
            <div class="space-y-3">
              <h4 class="font-black text-slate-900 dark:text-white text-base flex items-center gap-2">
                <svg lucideIcon="map" class="w-5 h-5 text-amber-500"></svg>
                <span>مخطط الممرات وتوجيه الحركة داخل المصنع والمخازن الهندسية</span>
              </h4>
              ...
              <div class="relative bg-slate-950 rounded-3xl p-6 overflow-hidden min-h-[340px] border border-slate-800 flex flex-col justify-between">
                ...
                <div class="my-8 p-6 bg-slate-900/90 rounded-2xl border border-amber-500/50 flex flex-wrap justify-between items-center gap-4">
                  <div class="flex items-center gap-3">
                    <div class="w-12 h-12 rounded-2xl bg-amber-500 text-slate-950 flex items-center justify-center font-black animate-bounce shadow-lg shrink-0">
                      <svg lucideIcon="navigation" class="w-6 h-6"></svg>
                    </div>
                    <div>
                      <p class="text-xs text-amber-300 font-bold">المسار الموصى به للوصول للصنف بالمصنع:</p>
