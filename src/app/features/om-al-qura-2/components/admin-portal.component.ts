...
import { OmAlQura2Service, OmAlQura2Employee, OmAlQura2AisleConfig, OmAlQura2Product, OmAlQura2Supplier, OmAlQura2PurchaseOrderItem } from '../../../core/services/om-al-qura-2.service';
import { ImageFallbackDirective } from '../../../shared/directives/image-fallback.directive';

@Component({
  selector: 'app-om-al-qura-2-admin-portal',
  standalone: true,
    ...
    <div class="space-y-8 font-sans" dir="rtl">
      ...
      <div class="grid grid-cols-1 md:grid-cols-4 gap-4">
        ...
        <div class="bg-gradient-to-br from-slate-900 to-slate-950 text-white rounded-3xl p-6 border border-slate-800 shadow-xl">
          ...
          <p class="text-[11px] text-rose-400 mt-2 font-bold">قطاعات ومعادن تحتاج إعادة توريد وشراء فوري</p>
        </div>
      </div>
        ...
        <div class="flex flex-wrap justify-between items-center gap-4 border-b border-slate-100 dark:border-slate-800 pb-4">
          <div>
            <div class="flex items-center gap-2">
              <span class="px-3 py-1 rounded-full text-xs font-black bg-amber-500/20 text-amber-300 border border-amber-400/30">
                صلاحيات مدير المصنع والإدارة
              </span>
            </div>
          ...
          <div class="flex items-center gap-3">
            <button (click)="showHireModal.set(true)"
                    class="px-5 py-2.5 rounded-2xl bg-gradient-to-r from-amber-600 to-amber-700 hover:from-amber-500 hover:to-amber-600 text-slate-950 font-black text-sm shadow-md transition-all flex items-center gap-2">
              <svg lucideIcon="user-plus" class="w-5 h-5"></svg>
              <span>+ تعيين عامل/مهندس جديد</span>
            </button>
        ...
        <div class="overflow-x-auto">
          <table *ngIf="service.employees().length > 0; else emptyEmployees" class="w-full text-sm text-right">
            <thead class="bg-slate-100 dark:bg-slate-800 text-slate-700 dark:text-slate-300 font-bold">
              <tr>
                <th class="p-3 rounded-r-xl">اسم المهندس / العامل</th>
      ...
      <!-- SECTION 2: Delivery Drivers Management (سائقي الشحن والتوريد) -->
      <div class="bg-white dark:bg-slate-900 rounded-3xl p-6 border border-slate-200 dark:border-slate-800 shadow-sm space-y-6">
        <div class="flex flex-wrap justify-between items-center gap-4 border-b border-slate-100 dark:border-slate-800 pb-4">
          <div>
            <h2 class="text-xl font-black text-slate-900 dark:text-white flex items-center gap-2">
              <svg lucideIcon="truck" class="w-6 h-6 text-amber-500"></svg>
              <span>إدارة طاقم سائقي التوريد والشحن الثقيل</span>
            </h2>
            <p class="text-xs text-slate-500 mt-1">تحديد ومتابعة سائقي وشاحنات النقل والتوريد بالمصنع وتوزيع المواعيد.</p>
          </div>

          <button (click)="showHireDriverModal.set(true)"
                  class="px-5 py-2.5 rounded-2xl bg-gradient-to-r from-amber-600 to-amber-700 hover:from-amber-500 hover:to-amber-600 text-slate-950 font-black text-sm shadow-md transition-all flex items-center gap-2">
            <svg lucideIcon="plus" class="w-5 h-5"></svg>
            <span>+ إضافة سائق شاحنة توريد جديد</span>
          </button>
        </div>
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
            ...
            <div>
              <div class="flex justify-between items-center">
                ...
                <span class="px-2.5 py-0.5 rounded-full text-xs font-bold"
                      [ngClass]="drv.status === 'متاح' ? 'bg-emerald-600 text-white' : 'bg-amber-500 text-slate-950'">
                  {{ drv.status }}
                </span>
              </div>
          ...
          <div *ngIf="service.deliveryDrivers().length === 0" class="col-span-full py-8 text-center text-slate-400 text-xs font-bold bg-slate-50 dark:bg-slate-800/50 rounded-2xl border border-slate-200 dark:border-slate-700">
            لم يتم تعيين أي سائقين أو شاحنات توريد بالمصنع حتى الآن
          </div>
        </div>
      </div>
      ...
      <div class="bg-white dark:bg-slate-900 rounded-3xl p-6 border border-slate-200 dark:border-slate-800 shadow-sm space-y-6">
        <div class="flex flex-wrap justify-between items-center gap-4 border-b border-slate-100 dark:border-slate-800 pb-4">
          <div>
            <h2 class="text-xl font-black text-slate-900 dark:text-white flex items-center gap-2">
              <svg lucideIcon="map-pin" class="w-6 h-6 text-amber-500"></svg>
              <span>تعديل المخطط الهيكلي للمصنع وتسمية الممرات والرفوف</span>
            </h2>
            <p class="text-xs text-slate-500 mt-1">تحديد أسماء الممرات، الرفوف، المدخل الرئيسي، وملاحظات السير للعملاء ومهندسي المصنع.</p>
          </div>

          <button (click)="openLayoutModal()"
                  class="px-5 py-2.5 rounded-2xl bg-gradient-to-r from-amber-600 to-amber-700 hover:from-amber-500 hover:to-amber-600 text-slate-950 font-black text-sm shadow-md transition-all flex items-center gap-2">
            <svg lucideIcon="edit-3" class="w-5 h-5"></svg>
            <span>تعديل المخطط الهيكلي للمصنع</span>
          </button>
        </div>
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-4 text-xs">
          ...
          <div class="p-4 rounded-2xl bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 space-y-1">
            <span class="text-slate-400 font-bold">منطقة الإدارة والمبيعات:</span>
            <p class="font-black text-slate-900 dark:text-white text-sm">{{ service.storeLayout().checkoutAreaLabel }}</p>
          </div>

          <div class="p-4 rounded-2xl bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 space-y-1">
            <span class="text-slate-400 font-bold">المخزن الهندسي للمعادن:</span>
            <p class="font-black text-slate-900 dark:text-white text-sm">{{ service.storeLayout().warehouseAreaLabel }}</p>
          </div>
        </div>

        <div class="space-y-2">
          <h4 class="font-black text-xs text-slate-800 dark:text-white">الممرات والرفوف المسجلة بالمصنع حالياً:</h4>
          <div class="grid grid-cols-1 md:grid-cols-3 gap-3">
            <div *ngFor="let aisle of service.storeLayout().aisles" class="p-4 rounded-2xl bg-amber-500/10 dark:bg-amber-950/40 border-2 border-amber-500/40 space-y-2">
              <h5 class="font-black text-amber-900 dark:text-amber-300 text-sm flex items-center gap-1.5">
                <svg lucideIcon="layers" class="w-4 h-4 text-amber-600 dark:text-amber-400"></svg>
                <span>{{ aisle.name }}</span>
              </h5>
              <div class="text-[11px] text-slate-800 dark:text-slate-200 font-bold leading-relaxed">
                <span class="font-black text-amber-800 dark:text-amber-400">الرفوف:</span> {{ aisle.shelves.join(' • ') }}
              </div>
            </div>
          </div>
        </div>
      ...
      <div class="bg-white dark:bg-slate-900 rounded-3xl p-6 border border-slate-200 dark:border-slate-800 shadow-sm space-y-6">
        <div class="flex flex-wrap justify-between items-center gap-4 border-b border-slate-100 dark:border-slate-800 pb-4">
          ...
          <div class="flex items-center gap-2">
            <button (click)="openAddSupplierModal.set(true)"
                    class="px-4 py-2 rounded-2xl bg-zinc-800 hover:bg-zinc-700 text-white font-black text-xs shadow-sm transition-all flex items-center gap-1.5 cursor-pointer border border-amber-500/30">
              <svg lucideIcon="plus" class="w-4 h-4 text-amber-400"></svg>
        ...
        <div class="space-y-3">
          ...
          <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-3">
            ...
            <div *ngIf="service.lowStockProducts().length === 0" class="col-span-full p-4 bg-emerald-600 text-white rounded-2xl text-xs font-black flex items-center justify-center gap-2 shadow-sm">
              <svg lucideIcon="check-circle-2" class="w-5 h-5 text-white shrink-0"></svg>
              <span>✓ ممتاز! جميع القطاعات والمعادن بالمصنع متوفرة بمخزون كافٍ كحد أدنى.</span>
            </div>
          </div>
        </div>
      ...
      <div *ngIf="openLayoutEditorModal()" class="fixed inset-0 z-50 bg-black/60 backdrop-blur-xs flex items-center justify-center p-4">
        <div class="bg-white dark:bg-slate-900 rounded-3xl p-6 max-w-xl w-full font-sans dir-rtl space-y-4 border border-slate-200 dark:border-slate-800 shadow-2xl max-h-[90vh] overflow-y-auto custom-scrollbar">
          ...
          <form (ngSubmit)="saveLayoutForm()" class="space-y-4 text-xs">
            <div>
              <label class="block font-bold text-slate-700 dark:text-slate-300 mb-1">اسم المدخل الرئيسي للمصنع</label>
            ...
            <div>
              <label class="block font-bold text-slate-700 dark:text-slate-300 mb-1">اسم منطقة الإدارة والمبيعات والمعارض</label>
            ...
            <div>
              <label class="block font-bold text-slate-700 dark:text-slate-300 mb-1">اسم منطقة المخزن الهندسي للمعادن والمواد الخام</label>
            ...
            <div>
              <label class="block font-bold text-slate-700 dark:text-slate-300 mb-1">المخطط الهندسي الهيكلي للمصنع (صورة أو رسم مباشر)</label>
        ...
        <div class="bg-white dark:bg-slate-900 rounded-3xl p-6 max-w-md w-full font-sans dir-rtl space-y-4 border border-slate-200 dark:border-slate-800 shadow-2xl">
          ...
          <form (ngSubmit)="submitAddSupplierForm()" class="space-y-4 text-xs">
            ...
            <div>
              ...
              <input type="text" [(ngModel)]="newSupplierCompany" name="suppComp" required placeholder="مثال: شركة مصر والأهرام للمعادن والصلب"
                     class="w-full p-3 rounded-2xl bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 font-bold text-slate-900 dark:text-white">
            </div>
            ...
            <div>
              ...
              <input type="text" [(ngModel)]="newSupplierCategoriesStr" name="suppCats" placeholder="مثال: قطاعات الألومنيوم ، المواسير والحديد الصلب"
                     class="w-full p-3 rounded-2xl bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 font-bold text-slate-900 dark:text-white">
            </div>
...
export class OmAlQura2AdminPortalComponent {
  ...
  editNotes = '';
  editSketchUrl = '';
  editAisles: OmAlQura2AisleConfig[] = [];
  ...
  newSupplierCompany = '';
  newSupplierPhone = '';
  newSupplierCategoriesStr = 'سبائك ألومنيوم ، صاج صلب مجلفن ، إستانلس 304';
  ...
  submitAddSupplierForm() {
    ...
    this.newSupplierCategoriesStr = 'سبائك ألومنيوم ، صاج صلب مجلفن ، إستانلس 304';
  ...
  openCreatePoForSingleProduct(product: OmAlQura2Product) {
  ...
  submitCreatePo() {
    ...
    const itemsToRequest: OmAlQura2PurchaseOrderItem[] = [];
