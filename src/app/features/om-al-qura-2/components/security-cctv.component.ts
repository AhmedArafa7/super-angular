...
import { OmAlQura2Service, OmAlQura2CctvCamera } from '../../../core/services/om-al-qura-2.service';
import { ToastService } from '../../../core/services/toast.service';

export interface SecurityEventLog2 {
  id: string;
  time: string;
  cameraName: string;
...
@Component({
  selector: 'app-om-al-qura-2-security-cctv',
  standalone: true,
      ...
      <div class="bg-gradient-to-r from-slate-900 via-slate-950 to-slate-900 p-6 md:p-8 rounded-3xl border border-slate-800 shadow-2xl flex flex-col md:flex-row items-start md:items-center justify-between gap-6">
        <div>
          <div class="flex items-center gap-2 mb-2 flex-wrap">
            <span class="px-3 py-1 rounded-full text-xs font-black bg-rose-500/20 text-rose-400 border border-rose-500/30 flex items-center gap-1.5 animate-pulse">
              <span class="w-2.5 h-2.5 rounded-full bg-rose-500"></span>
              نظام ربط الكاميرات الحقيقية للمصنع (REAL HARDWARE CCTV)
            </span>
          ...
          <h2 class="text-2xl md:text-3xl font-black text-white flex items-center gap-3">
            ...
            <span>مركز مراقبة الكاميرات الحقيقية لمصنع محمود عرفه</span>
          </h2>
          <p class="text-xs text-slate-400 mt-1">ربط وتأطير كاميرات الموبايل والكمبيوتر المباشرة (WebCam) وأجهزة DVR/NVR وشبكات IP لمصنع المعارض والمعادن.</p>
        </div>
      ...
      <div class="bg-slate-900 rounded-3xl p-6 border border-slate-800 shadow-xl space-y-4 font-sans">
        <div class="flex items-center justify-between border-b border-slate-800 pb-4">
          <h3 class="text-base font-black text-white flex items-center gap-2">
            ...
            <span>سجل التنبيهات والأحداث الأمنية المباشرة للمصنع (Security Logs)</span>
          </h3>
      ...
      <div *ngIf="openAddModal()" class="fixed inset-0 z-50 bg-black/80 backdrop-blur-sm flex items-center justify-center p-4">
        <div class="bg-slate-900 rounded-3xl max-w-lg w-full border border-slate-800 shadow-2xl p-6 space-y-6 dir-rtl">
          ...
          <div class="space-y-4">
            ...
            <div>
              <label class="block text-xs font-bold text-slate-300 mb-1">موقع الكاميرا بالمصنع *</label>
              <input type="text" [(ngModel)]="newCamData.location" placeholder="مثال: صالة التقطيع CNC أو معرض المعادن" class="w-full p-3 rounded-xl bg-slate-950 border border-slate-800 text-white text-xs font-bold focus:border-emerald-500">
            </div>
...
export class OmAlQura2SecurityCctvComponent implements OnInit, OnDestroy, AfterViewInit {
  ...
  selectedCamera = signal<OmAlQura2CctvCamera | null>(null);
  ...
  eventLogs = signal<SecurityEventLog2[]>([
    { id: 'l1', time: new Date(Date.now() - 3 * 60000).toLocaleTimeString('ar-EG'), cameraName: 'مدخل المصنع', type: 'entry', description: 'دخول عميل جديد لمعرض المعادن' },
    { id: 'l2', time: new Date(Date.now() - 12 * 60000).toLocaleTimeString('ar-EG'), cameraName: 'صالة التقطيع CNC', type: 'motion', description: 'تشغيل خط التقطيع وشغل العينات' }
  ]);
  ...
  adjustPan(cam: OmAlQura2CctvCamera, deltaX: number, deltaY: number) {
  ...
  takeSnapshot(cam: OmAlQura2CctvCamera) {
    this.toast.show(`تم التقاط صورة وحفظ لقطة ثابتة من (${cam.name}) 📷`, 'success');
  }

  maximizeCamera(cam: OmAlQura2CctvCamera) {
