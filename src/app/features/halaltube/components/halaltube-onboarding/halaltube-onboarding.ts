...
@Component({
  selector: 'app-halaltube-onboarding',
  standalone: true,
  imports: [CommonModule, FormsModule, LucideAngularModule],
  templateUrl: './halaltube-onboarding.html',
  styleUrls: ['./halaltube-onboarding.scss']
})
export class halaltubeOnboardingComponent implements OnInit {
  ...
  async searchAndPreview() {
    ...
    try {
      await this.halaltube.search(query);
      this.searchResultsPreview.set(this.halaltube.searchResults().slice(0, 3));
    } finally {
