...
export class halaltubeHomeComponent implements OnInit {
  ...
  categories = halaltube_CATEGORIES;
  ...
  shortsList = computed(() => {
    return this.halaltube.allHomeContent().filter(v => checkIsShorts(v));
  });
  ...
  quranList = computed(() => {
    const fromFeed = this.halaltube.allHomeContent().filter(v => 
  ...
  allVaultItems = computed(() => {
    ...
    const formattedCached = cached.map(c => ({
      ...
      author: c.author || 'halaltube Offline',
      quality: c.quality,
  ...
  ngOnInit() {
    if (this.needsOnboarding() && this.halaltube.allHomeContent().length === 0) {
    ...
    const savedShortsRows = localStorage.getItem('halaltube-shorts-rows');
    if (savedShortsRows) {
  ...
  async loadOfflineCachedMeta() {
    try {
      ...
      console.warn('[halaltubeHome] Offline cache load error:', e);
    }
  }
  ...
  increaseShortsRows() {
    this.shortsRows.update(r => {
      ...
      localStorage.setItem('halaltube-shorts-rows', val.toString());
      return val;
    });
  }
  ...
  onCategoryClick(category: string) {
    this.halaltube.setActiveCategory(category);
    if (category === 'الكل') {
      this.halaltube.setSearchQuery('');
      this.halaltube.searchSp.set('');
      ...
      this.halaltube.search(category);
    }
  }

  onSearchFilterClick(sp: string) {
    const query = this.halaltube.searchQuery();
    if (query) {
      this.halaltube.search(query, sp);
    }
  }
