...
export class WatchActionsComponent {
  ...
  getSubscriberLabel(): string {
    ...
    if (this.isOfficialCreator) {
      return 'منشئ محتوى موثق في halaltube Studio ⚡';
    }
    if (this.isYoutube) {
      return 'فيديو مُقترَح من مجتمع halaltube 👥';
    }
    return 'قناة في halaltube';
  }
  ...
  getAvatar(): string {
    ...
    if (name) {
      const subs: any[] = (this.halaltube as any).subscriptions ? (this.halaltube as any).subscriptions() : [];
      ...
      const homeContent: any[] = (this.halaltube as any).allHomeContent ? (this.halaltube as any).allHomeContent() : [];
