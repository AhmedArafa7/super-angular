...
function generateDobbleDeck(itemsCount = 8) {
    ...
    const shuffledEmojis = shuffleArray([...pool]);
    const generatedCards = cards.map(cardIndexes => cardIndexes.map(idx => shuffledEmojis[idx % shuffledEmojis.length]));
...
// RADIAL EMOJI PLACEMENT ENGINE WITH DYNAMIC RANDOMIZATION
function renderCircularCard(containerId, cardArray, playerSlot) {
    const container = document.getElementById(containerId);
    if (!container) return;
    container.innerHTML = '';

    // Shuffle the card array for display so matching elements appear at completely random positions
    const displayItems = shuffleArray([...cardArray]);

    const total = displayItems.length;
    const ringCount = total - 1;
    const radiusPercent = total >= 9 ? 34 : 32;

    // Center Emoji (randomized item)
    createEmojiNode(container, displayItems[0], 50, 50, playerSlot);
    ...
    for (let i = 1; i < total; i++) {
        ...
        createEmojiNode(container, displayItems[i], x, y, playerSlot);
    }
}
...
function createEmojiNode(container, emoji, xPercent, yPercent, playerSlot) {
    ...
    const fontPx = Math.floor(Math.min(window.innerHeight * 0.055, 42));
    span.style.fontSize = `${fontPx}px`;

    // Increased scale range (0.95x to 1.45x)
    const randomScale = (0.95 + Math.random() * 0.5).toFixed(2);
